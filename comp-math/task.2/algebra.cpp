...
matrix::matrix(int x_size_, int y_size_, std::vector<double> content_array_) : x_size(x_size_), y_size(y_size_), content_array(content_array_) {
    if (x_size * y_size != content_array.size()) {
        ...
        for (int j = 0; j < y_size; j++) {
            ...
            for (int i = 0; i < x_size; i++) {
                content[j][i] = content_array[i + j * x_size];
            }
        }
    }
}
...
std::vector<double> matrix::get_content_array() {
...
matrix matrix::transposition() {
    ...
    for (int j = 0; j < y_size; j++) {
        for (int i = 0; i < x_size; i++) {
            t_content[j+i*y_size] = content[j][i];
        }
    }
    return matrix(y_size, x_size, t_content);
}
